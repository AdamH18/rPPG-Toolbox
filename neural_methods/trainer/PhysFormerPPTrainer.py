...
class PhysFormerPPTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            self.model = ViT_ST_ST_Compact3_TDC_PP_gra_sharp(
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                gra_sharp = 2.0
                rPPG, ss = self.model(data, gra_sharp)
                ...
                for bb in range(data.shape[0]):
                    ...
                    attn_loss += TorchLossComputer.attention_loss(ss[bb], label[bb].to("cpu"), self.device)
                ...
                loss = a*loss_rPPG + b*(fre_loss+kl_loss) + attn_loss
                loss.backward()
                self.optimizer.step()
                ...
                loss_attn_avg.append(float(attn_loss.data))
                ...
                if idx % 100 == 99:  # print every 100 mini-batches
                    print(f'\nepoch:{epoch}, batch:{idx + 1}, total:{len(data_loader["train"]) // self.batch_size}, '
                        ...
                        f'Attn:{np.mean(loss_attn_avg[-2000:]):.3f}, hr_mae:{np.mean(loss_hr_mae[-2000:]):.3f}')

"""The dataloader for UBFC-rPPG1 dataset.

Details for the UBFC-rPPG1 Dataset see https://sites.google.com/view/ybenezeth/ubfcrppg.
...
class UBFCrPPG1Loader(BaseLoader):
    ...
    def get_raw_data(self, data_path):
        ...
        data_dirs = glob.glob(data_path + os.sep + "*-gt")
        if not data_dirs:
            raise ValueError(self.dataset_name + " data paths empty!")
        data_dirs.append(data_path + os.sep + "after-exercise")
        dirs = [{"index": data_dir.split(os.sep)[-1], "path": data_dir} for data_dir in data_dirs]
        return dirs
    ...
    def preprocess_dataset_subprocess(self, data_dirs, config_preprocess, i, file_list_dict):
        ...
        if config_preprocess.USE_PSUEDO_PPG_LABEL:
            ...
            bvps = self.read_wave(
                os.path.join(data_dirs[i]['path'],"gtdump.xmp"), frames.shape[0])
    ...
    @staticmethod
    def read_wave(bvp_file, frame_len):
        """Reads a bvp signal file."""
        with open(bvp_file, "r") as f:
            str1 = f.read()
            str1 = str1.split("\n")
            bvp = [float(x.split(',')[3]) for x in str1[:-1]]
